...
public class ChampagneTower {
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        System.out.println(champagneTower(poured2, query_row2, query_glass2));
        System.out.println(champagneTower(poured3, query_row3, query_glass3));
    }
    ...
    private static double champagneTower(int poured, int query_row, int query_glass) {
        ...
        for (int i = 1; i <= query_row; i++) {
            ...
            for (int j = 0; j < i; j++) {
                if (row[j] > 1) {
                    nextRow[j] += (row[j] - 1) / 2.0;
                    nextRow[j + 1] += (row[j] - 1) / 2.0;
                }
            }
            row = nextRow;
        }
